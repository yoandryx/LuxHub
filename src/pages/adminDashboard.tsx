...
import { updateNftMetadata, updateMarketStatus } from "../utils/metadata";
...
interface SaleRequest {
  ...
  seller: string;
  seed: number;
  initializerAmount: number; // SOL amounts
  takerAmount: number;       // SOL amounts
  fileCid: string;
  salePrice: number;         // SOL amounts
...
const updateNFTMarketStatus = async (
  ...
  try {
    ...
    if (!res.ok) {
      throw new Error("Failed to fetch current metadata from NFT.uri");
    }
    ...
    if (!wasUpdated) {
      console.log("[updateNFTMarketStatus] 'Market Status' attribute not found. Adding it...");
    ...
    console.log("[updateNFTMarketStatus] New metadata URI received:", newUri);
    ...
    await updateNftMetadata(wallet as any, mintAddress, { uri: newUri, name: newName });
...
const AdminDashboard: React.FC = () => {
  ...
  const fetchActiveEscrows = async () => {
    ...
    try {
      ...
      const mapped = escrows.map((acc: any) => ({
        ...
        initializer_amount: acc.account.initializer_amount ? acc.account.initializer_amount.toString() : "0",
  ...
  const initializeEscrowConfig = async () => {
    ...
    try {
      ...
      console.error("[initializeEscrowConfig] Initialization error:", error);
  ...
  const updateEscrowConfig = async () => {
    ...
    try {
      console.log("[updateEscrowConfig] Updating escrow config to new LuxHub wallet:", newLuxhubWallet);
      ...
      console.error("[updateEscrowConfig] Update error:", error);
  ...
  const addAdmin = async () => {
    ...
    try {
      ...
      console.error("[addAdmin] Add admin error:", error);
  ...
  const removeAdmin = async () => {
    ...
    try {
      ...
      console.error("[removeAdmin] Remove admin error:", error);
  ...
  const confirmDelivery = async (escrow: EscrowAccount) => {
    ...
    try {
      console.log("[confirmDelivery] Confirming delivery for escrow with seed:", escrow.seed);
  ...
  const handleApproveSale = async (req: SaleRequest) => {
    ...
    if (!req.seller) {
      console.error("[handleApproveSale] Seller field is missing in the sale request:", req);
      setStatus("Error: Seller field is missing in the sale request.");
      return;
    }
    try {
      console.log("[handleApproveSale] Approving listing for NFT mint:", req.nftId);
      const sellerPk = new PublicKey(req.seller);
      const seed = Number(req.seed);
      console.log("[handleApproveSale] Using seed:", seed);
      ...
      console.log("[handleApproveSale] Converted amounts (lamports):", { initializerLamports, takerLamports, salePriceLamports });
      ...
      let preIx: any[] = [];
      if (!sellerAtaAInfo) {
        console.log("[handleApproveSale] Seller ATA not found; creating new ATA...");
      ...
      const tx = await program!.methods.initialize(
        ...
        .rpc();
      console.log("[handleApproveSale] Escrow initialization successful. Tx:", tx);
      ...
      console.error("[handleApproveSale] Approve sale error:", error);
    ...
    switch (tabIndex) {
      ...
      case 1:
        return (
          <div>
            <h2>Admin List</h2>
            {adminList.length === 0 ? (<p>No admins found.</p>) : (adminList.map((adminStr, idx) => (<div key={idx} className={styles.listItem}>{adminStr}</div>)))}
          </div>
        );
      case 2:
        return (
          <div>
            ...
            {activeEscrows.length === 0 ? (<p>No active escrows found.</p>) : (activeEscrows.map((escrow, idx) => (
              <div key={idx} className={styles.listingCard}>
                ...
                <p><strong>Sale Price:</strong> {escrow.salePrice} (lamports)</p>
                <p><strong>File CID:</strong> {escrow.file_cid}</p>
                <div className={styles.actions}>
                  <button onClick={() => {/* confirm delivery logic if needed */}}>Confirm Delivery</button>
      ...
      case 4:
        return (
          <div>
            ...
            {logs.length === 0 ? (<p>No on‑chain event logs available.</p>) : (
              logs.map((log, index) => (
            ...
            {saleRequests.length === 0 ? (<p>No pending sale requests.</p>) : (
              saleRequests.map((req, idx) => (
                <div key={idx} className={styles.listingCard}>
                  ...
                  <button disabled={!req.seller} onClick={() => handleApproveSale(req)}>Approve Sale</button>
                </div>
              ))
            )}
          </div>
      ...
      {!wallet.publicKey ? (
        ...
        <>
          <div className={styles.tabContainer}>
            <button onClick={() => setTabIndex(0)} className={tabIndex === 0 ? styles.activeTab : ""}>Configuration</button>
            <button onClick={() => setTabIndex(1)} className={tabIndex === 1 ? styles.activeTab : ""}>User Management</button>
            <button onClick={() => setTabIndex(2)} className={tabIndex === 2 ? styles.activeTab : ""}>Escrow Management</button>
            <button onClick={() => setTabIndex(3)} className={tabIndex === 3 ? styles.activeTab : ""}>Detailed Analytics</button>
            <button onClick={() => setTabIndex(4)} className={tabIndex === 4 ? styles.activeTab : ""}>Transaction Logs</button>
            <button onClick={() => setTabIndex(5)} className={tabIndex === 5 ? styles.activeTab : ""}>Sale Requests</button>
          </div>
