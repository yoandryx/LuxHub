...
const adminKeypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(process.env.ADMIN_SECRET)));
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const updatedMetadataUri = await uploadToPinata(
      jsonData,
      jsonData.name || 'Updated NFT Metadata'
    );
    ...
    const enrichedNFT = {
      ...
      currentOwner:
        jsonData.attributes?.find((attr: any) => attr.trait_type === 'Current Owner')?.value || '',
