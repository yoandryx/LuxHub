...
- [ ] **FLOW-01**: Buyer can browse marketplace, click Buy, pay with USDC, and escrow status changes to funded
- [ ] **FLOW-02**: Buyer can browse marketplace, click Buy, pay with SOL (Jupiter swap), and escrow status changes to funded
- [ ] **FLOW-03**: Vendor receives notification when order is funded and can view order in vendor dashboard
- [ ] **FLOW-04**: Vendor can submit shipment with tracking info and buyer receives shipping notification
- [ ] **FLOW-05**: Buyer can confirm delivery and funds are released (97% vendor, 3% treasury)
- [ ] **FLOW-06**: Buyer can make offer on a listing and vendor receives offer notification
- [ ] **FLOW-07**: Vendor can accept, reject, or counter an offer from vendor dashboard
- [ ] **FLOW-08**: Buyer can respond to counter-offer (accept, reject, counter again)
- [ ] **FLOW-09**: When offer is accepted, buyer can pay within 24h deadline and escrow becomes funded
- [ ] **FLOW-10**: Other buyers' offers are auto-rejected when item is purchased, and they are notified
- [ ] **FLOW-11**: Vendor can onboard through 3-step wizard and admin can approve/reject
- [ ] **FLOW-12**: Approved vendor can list watches with images and metadata from vendor dashboard
- [ ] **FLOW-13**: Vendor dashboard shows orders tab and offers tab with real data and actions
- [ ] **FLOW-14**: Buyer orders page shows all orders with status, tracking, and action buttons
- [ ] **FLOW-15**: Pool token launch flow works (tokenize watch → bonding curve trading → fee share distribution)
- [ ] **FLOW-16**: Pool trading (buy/sell on bonding curve) reflects accurate pricing and balances
- [ ] **FLOW-17**: Notification bell shows unread count and notification list displays all lifecycle events

### Security Hardening

- [ ] **SEC-01**: All Solana connections use centralized cluster config — no hardcoded devnet fallbacks
- [ ] **SEC-02**: `_app.tsx` network selection is environment-driven (not hardcoded Devnet)
- [ ] **SEC-03**: TX verification validates program called, amount transferred, correct USDC mint, and escrow PDA destination
- [ ] **SEC-04**: TX verification prevents replay attacks (processed txSignatures stored and checked)
- [ ] **SEC-05**: Sentry error monitoring configured with DSN, source maps, and alerting
- [ ] **SEC-06**: Priority fee logic added to all fund-moving transactions for mainnet readiness
- [ ] **SEC-07**: Rate limiting applied to `/api/pool/invest` and `/api/pool/buy-resale`
- [ ] **SEC-08**: RPC connection fails loudly with clear error when endpoint env var is missing (no silent devnet fallback)

### Notification Fixes

- [ ] **NOTF-01**: Dispute creation notifies admins with correct notification type (not `shipment_submitted`)
- [ ] **NOTF-02**: Auto-rejected offers notify affected buyers with reason
- [ ] **NOTF-03**: Vendor delist requests notify admins
- [ ] **NOTF-04**: Shipment proof submission notifies admins for verification queue
...
| FLOW-15 | Phase 1 | Pending |
| FLOW-16 | Phase 1 | Pending |
| FLOW-17 | Phase 1 | Pending |
